...
// SAFE-3: the "waiting for a parent" content, extracted from PendingConsent.jsx so the
// standalone /pending-consent page and the inline gate on /miqte + /bisedat render the
// same explanation, the same parent-email hint, and the same resend button with its
// rate-limit handling — one implementation, not two that can drift.
//
// Every string here is the existing reviewed `pendingConsent.*` copy, reused verbatim.
//
// `showLogout` is on for the standalone page (where signing out is the only other way
// forward) and off for the inline gate, where the rest of the app is still usable and
// offering "log out" as the escape hatch would be actively misleading.
const ConsentPendingNotice = ({ showLogout = false }) => {
...
export default ConsentPendingNotice;
